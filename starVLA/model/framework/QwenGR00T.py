...
# 交接备注：
# 这里保留了一次将 VisualPatchCompressor 接到 QwenGR00T 中的尝试性修改。
# 当时的目标是在进入 GR00T action head 之前先压缩视觉 token 序列，以降低后续计算量。
# 代码已接入训练和推理链路，但当时实际运行时出现显存不足，因此没有完成完整实验验证。
class VisualPatchCompressor(nn.Module):
    def __init__(self, embed_dim, num_memory_tokens=64, num_heads=8):
        super().__init__()
        self.memory_tokens = nn.Parameter(torch.randn(1, num_memory_tokens, embed_dim))
    ...
    def forward(self, visual_features):
        ...
        query = self.memory_tokens.expand(batch_size, -1, -1)
        attn_output, _ = self.cross_attn(query, visual_features, visual_features)
...
class Qwen_GR00T(baseframework):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.config.framework.action_model.diffusion_model_cfg.cross_attention_dim = hidden_dim

        # 交接说明：这里尝试插入 VisualPatchCompressor，将长序列压缩为固定数量 token。
        # 当时使用 num_memory_tokens=64 作为实验值，尚未进一步配置化。
        self.compressor = VisualPatchCompressor(embed_dim=hidden_dim, num_memory_tokens=64)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 交接说明：原计划在进入 action model 前先压缩视觉 token，以减少序列长度。
        last_hidden = self.compressor(last_hidden)
